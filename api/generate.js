...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const variant of variants) {
      ...
      results.push({ variant, image: 'data:image/png;base64,' + imageB64, overlay_text: getOverlayText(topic, variant) });
    }
...
async function getGoogleToken(serviceAccount) {
  ...
  const payload = toB64Url(JSON.stringify({ iss: serviceAccount.client_email, scope: 'https://www.googleapis.com/auth/spreadsheets.readonly', aud: 'https://oauth2.googleapis.com/token', exp: now + 3600, iat: now }));
...
function findPattern(patterns, niche, contentFormat, emotionalHook) {
  ...
  for (const p of Object.values(patterns)) { if (p.niche === niche && p.content_format === contentFormat) return p; }
  for (const p of Object.values(patterns)) { if (p.niche === niche) return p; }
...
async function generatePrompt(apiKey, topic, niche, contentFormat, emotionalHook, pattern) {
  ...
  const prompt = 'YouTube thumbnail art director. Topic: ' + topic + '. Niche: ' + niche + '. Format: ' + contentFormat + '. Hook: ' + emotionalHook + '. Formula: ' + winningFormula + '. Write ONE Stability AI image prompt. Photorealistic 16:9. Person with ' + emotionalHook + ' expression. No text. Return ONLY the prompt.';
  const res = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' }, body: JSON.stringify({ model: 'claude-haiku-4-5', max_tokens: 300, messages: [{role:'user',content:prompt}] }) });
  const data = await res.json();
...
async function generateImage(apiKey, prompt) {
  const res = await fetch('https://api.stability.ai/v1/generation/stable-diffusion-xl-1024-v1-0/text-to-image', { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json', Authorization: 'Bearer ' + apiKey }, body: JSON.stringify({ text_prompts: [{text: prompt, weight: 1}], cfg_scale: 7, width: 1344, height: 768, steps: 30, samples: 1 }) });
  if (!res.ok) { const err = await res.text(); throw new Error('Stability error: ' + res.status + ' ' + err.substring(0,200)); }
  const data = await res.json();
