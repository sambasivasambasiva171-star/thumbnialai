...
async function generatePrompt(apiKey, topic, niche, contentFormat, emotionalHook, pattern) {
  const formula = pattern.winning_formula||'';
  const prompt = 'YouTube thumbnail. Topic:'+topic+'. Niche:'+niche+'. Hook:'+emotionalHook+'. Formula:'+formula+'. Write ONE Stability AI prompt. Photorealistic 16:9. Person with '+emotionalHook+' expression. No text. Return ONLY the prompt.';
  const res = await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':apiKey,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:'claude-haiku-4-5',max_tokens:200,messages:[{role:'user',content:prompt}]})});
  const data = await res.json();
...
async function generateImage(apiKey, prompt) {
  ...
  if (!res.ok){const err=await res.text();throw new Error('Stability:'+res.status+' '+err.substring(0,200));}
  const data = await res.json();
